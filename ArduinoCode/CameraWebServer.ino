...
void setup() {
  ...
  //các cảm biến ban đầu được lật theo chiều dọc và màu sắc hơi bão hòa
  if (s->id.PID == OV3660_PID) {
    s->set_vflip(s, 1);//flip it back
    s->set_brightness(s, 1);//up the blightness just a bit
    s->set_saturation(s, -2);//lower the saturation
  }
  //thả xuống kích thước khung hình để có tốc độ khung hình ban đầu cao hơn
...
void loop() {
  // đặt mã ID chính của bạn ở đây, để chạy nhiều lần:
  delay(1000);
